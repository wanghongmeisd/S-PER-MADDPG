...
def critic_updator_ddpg_wholestate(agents,num, state, whole_state, action,whole_action,reward, next_state,next_whole_state, done_value, gamma, ):
    value = agents[num].functor_dict['critic'](whole_state, whole_action)
    # print(value.mean())
    with torch.no_grad():
        for i, agent in enumerate(agents):
            next_action = agent.functor_dict['actor_target'](whole_state[18*i:17*(i+1)])
        target_next_value = agent.functor_dict['critic_target'](next_whole_state, next_wholeaction)
        #print('reward', reward)
        #print(done_value)
        target_value = reward + gamma * (done_value) * target_next_value
    ...
    value_loss = nn.MSELoss()(value, target_value)
    # print('loss',value_loss)
    agent1.optimizer_dict['critic'].zero_grad()
    value_loss.backward()
    agent1.optimizer_dict['critic'].step()
    # for name, parms in agent.critic.named_parameters():
    #     print('-->name:', name, '-->grad_requirs:', parms.requires_grad, \
    #           ' -->grad_value:', parms.grad)
...
def actor_updator_ddpg_wholestate(agent, whole_state ,whole_action ):
    for p in agent.functor_dict['critic'].parameters():
        p.requires_grad = False

    policy_loss = - agent.functor_dict['critic'](whole_state, whole_action).mean()
    # print(policy_loss)
