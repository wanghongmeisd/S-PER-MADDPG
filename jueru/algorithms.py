        ...
        while step <= (num_train_step):
            ...
            while True:
                ...
                if done:
                    ...
                    # with open('singleagent_rewards.txt', 'a') as file:
...
class MAAlgorithm:
    def __init__(
        ...
        if data_collection_args_dict is None:
            ...
            # for agent_name in range(self.env.purnum):
        ...
        if agent_args:
            ...
            for agent_class, actor, critic, discriminator, in zip(agent_class_list, actor_list, critic_list,
                                                                  discriminator_list):
                functor_dict={'actor': actor, 'critic': critic, 'discriminator': discriminator}
                lr_dict={'actor':self.lr_actor,'critic':self.lr_critic,'discriminator':self.lr_discriminator}
                optimizer_dict={'actor':self.optimizer_actor,'critic':self.optimizer_critic,'discriminator':self.optimizer_discriminator}
                self.agent_list.append(agent_class(functor_dict,
                                          optimizer_dict,
                                          lr_dict
                                           ))
            ...
            for agent_name, agent_class, actor, critic, data_collection, discriminator in zip(self.env.possible_agents,
                ...
                #                                    lr_discriminator=lr_discriminator,
                #                                    ))
                functor_dict = {'actor': actor, 'critic': critic}
                lr_dict = {'actor': self.lr_actor,
                           'critic':self.lr_critic, 'discriminator':self.lr_discriminator}
                optimizer_dict = {'actor': self.optimizer_actor,
                                  'critic':self.optimizer_critic, 'discriminator':self.optimizer_discriminator}
                self.agent_list.append(agent_class(functor_dict,
                                                   optimizer_dict,
                                                   lr_dict
                                                   ))
    ...
    def learn(self, num_train_step):
        ...
        for agent in self.agent_list:
            agent.functor_dict['actor'].train()
        ...
        while step <= (num_train_step):
            ...
            for agent_num in self.env.agents:
                # print(agent_name)
                if self.render:
                    self.env.render()
                ...
                if step >= self.start_steps:
                    ...
                    action = self.env.action_spaces[agent_num].sample()
                # print(action)
                # if before_done:
                #     action = None
                # print(action)
                ...
                total_reward += reward
                self.data_collection_list[agent_num].store(state, action, reward, s_, done)
