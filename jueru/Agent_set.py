...
class DDPG_agent(Agent):
    ...
    def choose_action(self, observation: object, noise_scale: object) -> object:
        ...
        with torch.no_grad():
            ...
            # a = a.squeeze(0).numpy()
            a_cpu = a.cpu()
            a_cpu += noise_scale * np.random.randn(self.functor_dict['actor'].action_dim)
            a_cpu = a_cpu.squeeze(0).numpy()  # 转成数组
        # print(self.functor_dict['actor'].limit_low)
        return np.clip(a_cpu, self.functor_dict['actor'].limit_low.cpu(), self.functor_dict['actor'].limit_high.cpu()).numpy()
