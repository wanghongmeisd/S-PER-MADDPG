...
class Replay_buffer:
    '''multi_agent replay_buffer,增加了全局状态whole_state_buf'''
    def __init__(self, env, size, device: Union[torch.device, str] = "auto"):
        self.obs_shape = get_obs_shape(env.observation_space)
        self.virleader_obs_shape =  get_obs_shape(env.virleader_observation_space)
        ...
        self.reward_buf = np.zeros((int(size),1), dtype=np.float32) #reward 是多个agentreward列表
        self.done_buf = np.zeros((int(size), 1), dtype=np.float32)

        self.ptr, self.size, self.max_size = 0, 0, size
        # 多个agent情况,包括virleader
        state_num = env.purnum
        next_state_num = self.obs_shape[0] * (state_num - 1) + self.virleader_obs_shape[0]
        self.next_whole_state_buf = torch.zeros((int(size), next_state_num),
                                             dtype=torch.float32).to(self.device)

        self.whole_done_buf = torch.zeros((int(size),  state_num), dtype=torch.float32).to(self.device)
        self.whole_action_buf = torch.zeros((int(size), self.action_dim *  state_num), dtype=torch.float32).to(self.device)
        self.whole_state_buf = torch.zeros((int(size), next_state_num), dtype=torch.float32).to(self.device)
        self.whole_reward_buf = torch.zeros((int(size),  state_num), dtype=torch.float32).to(self.device)  # reward 是多个agentreward列表
    def store_whole_cuda(self, whole_obs,whole_act, whole_rew, next_whole_obs, whole_done):
        self.whole_state_buf[int(self.ptr)] = torch.tensor(whole_obs).clone()
        self.whole_action_buf[int(self.ptr)] = torch.tensor(whole_act).clone()
        self.whole_reward_buf[int(self.ptr)] = torch.tensor(whole_rew).clone()
        self.next_whole_state_buf[int(self.ptr)] = torch.tensor(next_whole_obs).clone()
        self.whole_done_buf[int(self.ptr)] = torch.tensor(whole_done).clone()
        self.ptr = (self.ptr+1) % self.max_size
        self.size = min(self.size+1, self.max_size)

    def store_whole(self, whole_obs, whole_act, whole_rew, next_whole_obs, whole_done):
        self.whole_state_buf[int(self.ptr)] = np.array(whole_obs).copy()
        self.whole_action_buf[int(self.ptr)] = np.array(whole_act).copy()
        self.whole_reward_buf[int(self.ptr)] = np.array(whole_rew).copy()
        self.next_whole_state_buf[int(self.ptr)] = np.array(next_whole_obs).copy()
        self.whole_done_buf[int(self.ptr)] = np.array(whole_done).copy()
        self.ptr = (self.ptr + 1) % self.max_size
        self.size = min(self.size + 1, self.max_size)
    ...
    def sample_batch_whole(self, batch_size=32):

        batch_size = min(batch_size, self.size)

        idxs = np.random.randint(0, self.size, size=batch_size)
        batch = dict(state=self.whole_state_buf[idxs],
                    action = self.whole_action_buf[idxs],
                     next_state=self.next_whole_state_buf[idxs],
                     reward=self.whole_reward_buf[idxs],
                     done=self.whole_done_buf[idxs]
                     )
        return {k: torch.as_tensor(v.clone(), dtype=torch.float32).to(self.device) for k,v in batch.items()}
